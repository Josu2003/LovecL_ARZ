...
local update_json_url = "https://raw.githubusercontent.com/kooradev/LovecL_ARZ/refs/heads/main/update.json"
local updateVersion = ""
                ...
                if f then
                    ...
                    if ok and data and data.version then
                        if tostring(data.version) ~= current_version then
                            ...
                            sampAddChatMessage(colors.turquoise .. nameScript .. colors.white .. " Äîñòóïíî íîâîå îáíîâëåíèå" .. colors.green .. " v" .. updateVersion .. colors.white .. "! Ââåäèòå" .. colors.red .. " /updatelovecl" .. colors.white .. ", ÷òîáû îáíîâèòüñÿ.", -1)
                        end
                    end
                end
            end
...
function startUpdate(download_url)
    ...
    downloadUrlToFile(download_url, script_path, function(id, status, p1, p2)
        if status == 6 then
            sampAddChatMessage(colors.turquoise .. nameScript .. colors.green .. " Ñêðèïò óñïåøíî îáíîâëåí! Ïåðåçàãðóçêà...", -1)
...
function main()
    ...
    sampAddChatMessage(colors.turquoise .. nameScript .. colors.white .. "Ñêðèïò çàãðóæåí! Âåðñèÿ: 2.1 | Àâòîð:" .. colors.turquoise .. " Koora")
    ...
    if updateVersion ~= "" then
        ...
        sampAddChatMessage(colors.turquoise .. nameScript .. colors.white .. " Îáíîâëåíèÿ íå íàéäåíû èëè ó âàñ óæå óñòàíîâëåíà ïîñëåäíÿÿ âåðñèÿ.", -1)
    end
    end)
...
imgui.OnFrame(function()
    ...
    imgui.Text(u8 "Ñêðèïò: LovecL ARZ v2.1")
    ...
    imgui.SameLine()
    imgui.Text(u8 " Ïðîâåðèòü îáíîâëåíèå ñêðèïòà.")
    ...
    imgui.BulletText(
        u8 "Â äàííîì ñêðèïòå ïðèñóòñòâóåò àâòî-îáíîâëåíèå ïîñëå ïîäòâåðæäåíèÿ.")
    imgui.BulletText(
